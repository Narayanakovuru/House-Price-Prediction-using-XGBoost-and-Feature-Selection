...
def predict_price(features: Features):
    try:
        # Initialize dataframe with all expected columns as NaN
        df = pd.DataFrame(columns=preprocessor.feature_names_in_)
        df.loc[0] = np.nan
        
        # Fill in the features provided by the frontend
        for col, val in features.data.items():
